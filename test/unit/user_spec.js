...
describe('User', function(){
  ...
  beforeEach(function(done){
    global.nss.db.dropDatabase(function(err, result){
      bob = new User({name:'The Best Bob', email:'bob@nomail.com', password:'1234'});
  ...
  describe('new', function(){
    it('should create a new User object', function(){
      var u1 = new User({name:'The Best Bob', email:'bob@nomail.com', password:'1234'});
      expect(u1).to.be.instanceof(User);
      expect(u1.email).to.equal('bob@nomail.com');
      expect(u1.password).to.equal('1234');
      expect(u1.name).to.equal('The Best Bob');
    });
  });

  describe('#register', function(){
    it('should register a new User', function(done){
      var u1 = new User({name:'nss tester', email:'pete-nss@outlook.com', password:'1234'});
